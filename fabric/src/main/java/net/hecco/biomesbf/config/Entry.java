...
public abstract class Entry<T> {
    ...
    public static Entry<Integer> integerEntry(String name, Supplier<Integer> current, Consumer<Integer> saver, Integer defaultValue, Integer min, Integer max, String... tooltip) {
        return new Entry<>(name, current, saver, defaultValue, min, max, tooltip) {
            ...
            public TooltipListEntry<Integer> build(ConfigEntryBuilder builder) {
                return builder.startIntSlider(Component.translatable(getText()), getCurrent().get(), getMin(), getMax())
                        .setSaveConsumer(getSaver())
                        .setTooltip(Arrays.stream(getTooltip()).map(Component::literal).toArray(Component[]::new))
                        .setDefaultValue(getDefaultValue()).setMin(getMin()).setMax(getMax())
                        .build();
    ...
    public static <T extends Enum<?>> Entry<T> enumEntry(String name, Supplier<T> current, Consumer<T> saver, Class<T> enumClass, T enumValue, String... tooltip) {
        return new Entry<T>(name, current, saver, enumClass, enumValue, tooltip) {
            @Override
            public TooltipListEntry<T> build(ConfigEntryBuilder builder) {
                return builder.startEnumSelector(Component.translatable(getText()), enumClass, enumValue)
                        .setSaveConsumer(getSaver())
                        .setTooltip(Arrays.stream(getTooltip()).map(Component::literal).toArray(Component[]::new))
                        .setDefaultValue(getDefaultValue())
                        .build();
    ...
    public Entry(String name, Supplier<T> current, Consumer<T> saver, Class<T> enumClass, T enumValue, String[] tooltip) {
        this(name, current, saver, enumValue, null, null, tooltip);
    }
